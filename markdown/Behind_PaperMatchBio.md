# Behind [PaperMatchBio](https://bio.papermatch.me/)
...
I also want the extracted data to be saved somewhere persistant, since [HuggingFace Spaces](https://huggingface.co/spaces) are ephemeral. For saving the data, we will again be using the [HuggingFace Datasets](https://huggingface.co/datasets).
...
bioRxiv stores their data in `.meca` files. Which are nothing but renamed `.zip` files. Does anyone know why not just use good ol' zip? Anywho, once you unzip the `.meca` files, you will find the contents. There will be an `XML` file with the full text in there sitting nicely, along with some figures which were used to create the final `PDF`. We only care about the `XML` at this point. So lets get that!
...
Once all the XML files have been extracted, we zip the output folder and upload to HF Datasets. However, we want to be doing this on HF Spaces, for that, we will create a new thread to let the process happed there:
...
Just create an app.py on HF Spaces, add the requirements file, add your credentials to the space, and see the data coming your way!

All of this code is available on [mitanshu7/tdm](https://github.com/mitanshu7/tdm).
...
Now we can merge both and have a final dataset that can be inserted into Milvus.
...
All of this code can be found at [mitanshu7/embed_xml](https://github.com/mitanshu7/embed_xml).

Phew! Now we follow a very similar setup to the one we saw in [Behind PaperMatch](Behind_PaperMatch.html).
